...
import 'package:ar_view/location_ar.dart';
...
class ObjectGesturesWidgetState extends State<ObjectGesturesWidget> {
          ...
          ElevatedButton(
              onPressed: () {
                ...
                Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(
                      builder: (context) => const LocationAR(),
                    ));
              },
  ...
  onPanStarted(String nodeName) {
    debugPrint("노드 시작... $nodeName");
  }

  onPanChanged(String nodeName) {
    debugPrint("노드 실행 중... $nodeName");
  }

  onPanEnded(String nodeName, Matrix4 newTransform) {
    debugPrint("노드 종료... $nodeName");
    final pannedNode = nodes.firstWhere((element) => element.name == nodeName);
  }

  onRotationStarted(String nodeName) {
    debugPrint("회전 노드 시작... $nodeName");
  }

  onRotationChanged(String nodeName) {
    debugPrint("회전 노드 실행 중... $nodeName");
  }

  onRotationEnded(String nodeName, Matrix4 newTransform) {
    debugPrint("회전 노드 종료... $nodeName");
